...
#Use vendored license file (place this under config/licenses/)
license_file "LICENSE-ADOPTIUM-GPLv2+CE"
...
# Always match extracted JDK folder structure
...
build do
  ...
  unless File.exist?("#{install_dir}/embedded/open-jre/lib/jli/libjli.so")
    raise "libjli.so is missing! Verify the JRE tarball structure."
  end

  # Correct RPATH so java binaries can resolve libjli.so
  new_rpath = [
    "#{install_dir}/embedded/open-jre/lib/jli",
